...
public class AsciiPanel extends JPanel {
	...
	public void paint(final Graphics g) {
		...
		for (int x = 0; x < widthInCharacters; x++) {
			for (int y = 0; y < heightInCharacters; y++) {
				...
				offscreenGraphics.drawImage(img, x * charWidth, y * charHeight,
						null);
	...
	private void loadGlyphs() {
		try {
			glyphSprite = ImageIO.read(AsciiPanel.class
					.getResource("/cp437.png"));
	 ...
	 * @return this for convenient chaining of method calls
	 */
	public AsciiPanel clear(final char character, final Color foreground,
	 ...
	 * @return this for convenient chaining of method calls
	 */
	public AsciiPanel clear(final char character, final int x, final int y,
	 ...
	 */
	public AsciiPanel clear(final char character, final int x, final int y,
			final int width, final int height, final Color foreground,
			final Color background) {
		if (character < 0 || character >= glyphs.length)
			throw new IllegalArgumentException("character " + character
					+ " must be within range [0," + glyphs.length + "].");
	...
	public AsciiPanel write(final String string, final Color foreground) {
		...
		return write(string, cursorX, cursorY, foreground,
				defaultBackgroundColor);
	}
	...
	public AsciiPanel write(final String string, final int x, final int y) {
		...
		if (x + string.length() >= widthInCharacters)
			throw new IllegalArgumentException("x + string.length() "
					+ (x + string.length()) + " must be less than "
					+ widthInCharacters + ".");
		...
		return write(string, x, y, defaultForegroundColor,
				defaultBackgroundColor);
	}
	...
	public AsciiPanel writeCenter(final String string, final int y) {
		...
		if (string.length() >= widthInCharacters)
			throw new IllegalArgumentException("string.length() "
					+ string.length() + " must be less than "
					+ widthInCharacters + ".");
	...
	public void withEachTile(final int left, final int top, final int width,
			final int height, final TileTransformer transformer) {
		...
		for (int x0 = 0; x0 < width; x0++) {
			for (int y0 = 0; y0 < height; y0++) {
				...
				if (x < 0 || y < 0 || x >= widthInCharacters
						|| y >= heightInCharacters) {
					continue;
				}
